...
void QHttpController::restRequest(){
    ...
    for(int i = 0; i < document.count(); i++){
        ...
        qDebug() << photo;
        if(userid == "Природа" && commentscount != 0 ) nature = commentscount;
        else if (userid == "Телефоны" && commentscount != 0 ) phone = commentscount;
        else if (userid == "Игры" && commentscount != 0 ) game = commentscount;
        else if (userid == "Машины" && commentscount != 0 ) car = commentscount;
        else if (userid == "Фото со мной" && commentscount != 0 ) avatar = commentscount;
        else newalbum = commentscount;
    ...
    emit toQML9(nature, phone, game, car, avatar /*,newalbum*/);
    qDebug() << nature<<"MORERE";
}
...
void QHttpController::db_write() { //функция вывода с бд
    QEventLoop loop;
    nam = new QNetworkAccessManager();
    ...
    db.open();
    QSqlQuery query;
    query.exec("DROP TABLE rut");//дроп таблицы
